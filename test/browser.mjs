...
try {
  const { result, stderr } = await runChromePage({
    ...
    screenshot,
    timeout: 120_000,
    url: `http://127.0.0.1:${address.port}/workbench.html?case=reference&eager-frames&mode=study&size=128`,
  ...
  if (
    ...
    throw new Error(`browser renderer failure\n${stderr}`);
  if (
    result.ready !== "true" ||
    result.specimens !== 4 ||
    result.mediaReports !== 4 ||
    result.mediaIssues !== 0 ||
    result.duplicateSvgIds !== 0
  )
    throw new Error(`workbench contract failed: ${JSON.stringify(result)}`);

  if ((await stat(screenshot)).size < 10_000)
    throw new Error("browser screenshot is unexpectedly empty");

  passed = true;
  console.log(
    "browser: every maintained species passed SVG, GL and media checks"
  );
} finally {
